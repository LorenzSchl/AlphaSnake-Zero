...
FOOD_REWARD = 3.5 # Reward for eating food / Growing in length
...
class Game:
    ...
    def get_states(self):
        ...
        states = [self.make_state_opt(snake, self.last_moves[snake.id]) for snake in self.snakes]
        #for i in range(len(states)):
        #    assert np.array_equal(states[i], states_opt[i])
        return states
    ...
    def make_state(self, you, last_move):
        ...
        grid = [[[0.0, 0.0, 0.0] for _ in range(width)] for _ in range(height)]
        center_y = height // 2
        center_x = width // 2
    ...
    def make_state_opt(self, you, last_move):
        # Initialization using numpy arrays
        width, height = self.width * 2 - 1, self.height * 2 - 1
        NUMBER_OF_VIEWS = 3
        grid = np.zeros((height, width, NUMBER_OF_VIEWS), dtype=np.float32)
        center_y, center_x = height // 2, width // 2

        board = np.zeros((self.height, self.width, NUMBER_OF_VIEWS), dtype=np.float32)

        # Precompute Manhattan distances
        y_grid, x_grid = np.ogrid[-center_y:center_y + 1, -center_x:center_x + 1]
        manhattan_distance = np.abs(y_grid) + np.abs(x_grid)

        length_minus_half = you.length - 0.5
        for snake in self.snakes:
            head_y, head_x = snake.head.position
            board[head_y, head_x, 0] = (snake.length - length_minus_half) * HEAD_m
        ...
        # Update grid using board and Manhattan distance
        within_radius = manhattan_distance <= 5
        within_radius_board = within_radius[center_y - self.height:center_y, center_x - self.width:center_x]

        grid[center_y - self.height:center_y, center_x - self.width:center_x][within_radius_board] = board[
            within_radius_board]

        # Update the grid with the snake's body parts
        for part in you:
            grid_y, grid_x = part[0] - head_y + center_y, part[1] - head_x + center_x
            if 0 <= grid_y < height and 0 <= grid_x < width:
                grid[grid_y, grid_x] = board[part[0], part[1]]
        return np.rot90(grid, k=last_move)
